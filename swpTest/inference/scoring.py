...
from config import LABELS, INV_LABELMAP, test_ids
...
def plot_confusion_matrix(y_true, y_pred, classes,
    ...
    # 将confusion保存到predictions/CFM文件夹中去
    ...
    fig.tight_layout()
    # save to directory
    if not os.path.isdir(base+'/'+savedir):
        os.mkdir(base+'/'+savedir)
    savefile = base+'/'+savedir+'/'+fname
...
def score_predictions(dataset, basedir='predictions'):
    ...
    for scene in test_ids:
        ...
        predsfile = os.path.join(dataset+'/'+basedir, f"{scene}-prediction.png")
