...
class fogAugmented(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        replacedName = self.root + 'test/'+ prefix_name+'_fog_'+str(self.severity)+suffix_name
        print(replacedName)
...
def save_fog(path,method=fog):
    for severity in range(1, 6):
        ...
        distorted_dataset_loader = torch.utils.data.DataLoader(
            distorted_dataset, batch_size=5, shuffle=False, num_workers=10)

        for _ in distorted_dataset_loader:
            continue

path = '/home/swp/IJAGCode/swpTest/rsData/multimodality/out/vaihingen'
# create the corrupted dataset with fog, image is in the style 'RGB', not RGBA
# save_fog(path,fog)
#%%
# convert the fog corrupted RGB image to RGBA(alpha channel with fog)
# from images in original 'val' dataset, we can get RGBA. from these images, we extract the alpha channel.
val_dataset = get_image_files('/home/swp/IJAGCode/swpTest/rsData/multimodality/out/vaihingen/img_dir/val')
test_dataset = get_image_files('/home/swp/IJAGCode/swpTest/rsData/multimodality/out/vaihingen/img_dir/test/alpha')

def convert2RGBA():
    for i in range(len(val_dataset)):
        pathName, fileName = os.path.split(val_dataset[i])
        prefix_name = os.path.splitext(fileName)[0]
        suffix_name = os.path.splitext(fileName)[1]

        for j in range(len(test_dataset)):
            pathName2, fileName2 = os.path.split(test_dataset[j])
            prefix_name2 = os.path.splitext(fileName2)[0]
            suffix_name2 = os.path.splitext(fileName2)[1]
            # test set find the corresponding file 
            if prefix_name in prefix_name2:
                # read the rgb fog file
                img = mmcv.imread(test_dataset[j],flag='unchanged')
                # convert it to rgba format
                rgba = cv2.cvtColor(img,cv2.COLOR_RGB2RGBA)
                # extract the alpha channel from file in the val dataset
                dsm = mmcv.imread(val_dataset[i],flag='unchanged')[:,:,3]
                rgba[:,:,3] = dsm
                mmcv.imwrite(rgba,pathName2+'/alpha/'+fileName2)
# convert2RGBA()                
...
def copyAndRename(path):
    ...
    for file in fileLists:
        ...
        suffix_name = os.path.splitext(fileName)[1]
        for i in range(5):
            newName = path+'test/'+prefix_name+'_fog_'+str(i+1)+suffix_name
            print(newName)
