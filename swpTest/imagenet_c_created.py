...
class DistortImageFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        Image.fromarray(np.uint8(img)).save(
            save_path, quality=85, optimize=True)
...
def glass_blur(x, severity=1):
    ...
    x = np.uint8(gaussian(np.array(x) / 255.,
                          sigma=c[0], multichannel=True) * 255)
...
def frost(x, severity=1):
    ...
    x_start, y_start = np.random.randint(
        0, frost.shape[0] - 224), np.random.randint(0, frost.shape[1] - 224)
...
def snow(x, severity=1):
    ...
    x = c[6] * x + (1 - c[6]) * np.maximum(x, cv2.cvtColor(x,
                                                           cv2.COLOR_RGB2GRAY).reshape(224, 224, 1) * 1.5 + 0.5)
...
def save_distorted(method=gaussian_noise):
    for severity in range(1, 6):
        ...
        distorted_dataset = DistortImageFolder(
            ...
            transform=trn.Compose([trn.RandomCrop(512)]))
